...
MANAGEMENT_PERMISSION_URL = f"{BASE_URL}/api/management/auth/permissions"
...
def test_list_permissions():
    ...
    url = f"{MANAGEMENT_PERMISSION_URL}/"
    print(f"请求URL: {url}")
...
def clean_test_permissions():
    ...
    # 获取权限列表
    try:
        response = requests.get(f"{MANAGEMENT_PERMISSION_URL}/", headers=headers)
        if response.status_code == 200:
            ...
            for permission in permissions:
                if 'test_service' in permission.get('service', ''):
                    ...
                    if permission_id:
                        delete_url = f"{MANAGEMENT_PERMISSION_URL}/{permission_id}/"
...
def test_retrieve_permission():
    ...
    url = f"{MANAGEMENT_PERMISSION_URL}/{test_data['created_permission_id']}/"
    print(f"请求URL: {url}")
